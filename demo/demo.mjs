...
async function session(label, actions) {
  ...
  for (const [tool, args, note] of actions) {
    ...
    const res = await rpc(child, id++, 'tools/call', { name: tool, arguments: args })
    const text = res.result?.content?.[0]?.text ?? JSON.stringify(res.error)
    let out = text
    try {
      const j = JSON.parse(text)
      if (tool === 'search_memory' && j.results) {
        out = j.results.slice(0, 1).map((r) => `  ${green('✓')} ${r.memory ?? r.content ?? JSON.stringify(r).slice(0, 80)}`).join('\n')
...
console.log(bold('$ npx @n3wth/r3') + dim('  # zero config: embedded Redis + local vectors'))

await session('Session 1', [
  ['add_memory', { content: 'Oliver prefers TypeScript with no semicolons', priority: 'high' }, 'memory stored'],
  ['add_memory', { content: 'Deploy target is Vercel, region sfo1' }, 'memory stored'],
])
